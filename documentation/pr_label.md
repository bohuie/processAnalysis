...
* `get_clean_pr_label.py` (final event timestamp rules) 
...
Drops rows where any *author-like* column is a bot account (PRs, commits, reviews).
...
* **`get_clean_pr_label.py`** — converts `pr_labels_{team}.csv` → `CLEAN_pr_labels_{team}.csv` and enforces timestamp rules. 
